// Groq Reverse Proxy - follows redirects and rewrites HTML/JS content URLs
// Usage:
//   /api/...        -> api.groq.com
//   /stytch/...     -> api.stytchb2b.groq.com
//   /anything-else  -> console.groq.com
...
async function handleRequest(request: Request): Promise<Response> {
  ...
  // Determine target host based on path prefix
  ...
  } else if (path.startsWith("/stytch")) {
    targetHost = STYTCH_HOST;
    targetPath = path.replace(/^\/stytch/, "") || "/";
  } else {
    targetHost = CONSOLE_HOST;
    targetPath = path;
  }
  ...
  try {
    ...
    // Rewrite HTML content to replace absolute URLs with proxy-relative paths
    if (contentType.includes("text/html")) {
      ...
      html = html.replace(/https:\/\/(cdn\.groq\.com)/g, "/cdn");
      html = html.replace(/https:\/\/(www\.groq\.com)/g, "/www");
    ...
    // Rewrite JS content to replace absolute URLs
    if (contentType.includes("javascript") || contentType.includes("application/json")) {
      let text = await response.text();
      text = text.replace(/https:\/\/(api\.groq\.com)/g, "https://groq-proxy.zhuimingggg.deno.net/api");
      text = text.replace(/https:\/\/(console\.groq\.com)/g, "https://groq-proxy.zhuimingggg.deno.net");
      text = text.replace(/https:\/\/(api\.stytchb2b\.groq\.com)/g, "https://groq-proxy.zhuimingggg.deno.net/stytch");
      text = text.replace(/https:\/\/(cdn\.groq\.com)/g, "https://groq-proxy.zhuimingggg.deno.net/cdn");
      text = text.replace(/https:\/\/([a-zA-Z0-9-]+)\.groq\.com/g, "https://groq-proxy.zhuimingggg.deno.net/$1");

      const respHeaders = new Headers();
      respHeaders.set("Content-Type", contentType);
      respHeaders.set("Content-Length", new TextEncoder().encode(text).length.toString());
      respHeaders.set("Access-Control-Allow-Origin", "*");
